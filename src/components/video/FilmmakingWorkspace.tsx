...
export function FilmmakingWorkspace({ prompt, onReturn, capabilities = VIDEO_CAPABILITIES, onScenePromptChange, onGenerateScene, onUseAsNextSceneFirstFrame, onSelectTake }: FilmmakingWorkspaceProps) {
  ...
  return (
    <section data-testid="video-studio" className="flex h-full min-h-0 flex-col bg-zinc-50 text-zinc-950">
      <header className="flex min-h-16 shrink-0 items-center gap-3 border-b border-zinc-200 bg-white px-4 sm:px-5">
        <button type="button" onClick={onReturn} aria-label="Back to editor" className="inline-flex h-9 items-center gap-2 rounded-full px-2 text-xs font-medium text-zinc-600 hover:bg-zinc-100 sm:px-3"><ArrowLeft size={16} /><span className="hidden sm:inline">Back to editor</span></button>
        <span className="flex h-9 w-9 items-center justify-center rounded-xl border border-zinc-200 text-violet-700"><FilmStrip size={19} /></span>
        ...
        <span className="hidden items-center gap-1.5 rounded-full bg-zinc-100 px-3 py-1.5 text-xs text-zinc-600 sm:inline-flex"><Queue size={14} /> {queueMessage}</span>
      </header>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto lg:grid lg:grid-cols-[220px_minmax(0,1fr)_360px] lg:overflow-hidden">
        ...
        <main className="min-w-0 shrink-0 p-3 sm:p-5 lg:min-h-0 lg:shrink lg:overflow-y-auto">
          <div className="mx-auto max-w-5xl space-y-4">
            <section className="overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-950 shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-2 border-t border-white/10 px-3 py-2.5 text-xs text-zinc-300 sm:px-4">
                ...
                <button type="button" onClick={() => void extractFrame('png', true)} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-2 hover:bg-white/10" data-testid="export-frame-png"><DownloadSimple size={14} /> PNG</button>
                <button type="button" onClick={() => void extractFrame('jpeg', true)} className="inline-flex h-8 items-center gap-1.5 rounded-lg px-2 hover:bg-white/10" data-testid="export-frame-jpg"><DownloadSimple size={14} /> JPG</button>
                <button type="button" onClick={useAsNextFirstFrame} disabled={!videoUrl} className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-violet-500/20 px-2.5 text-violet-200 hover:bg-violet-500/30 disabled:opacity-40" data-testid="use-next-first-frame"><ArrowRight size={14} /> Use as next first frame</button>
            ...
            <div className="flex gap-1 rounded-xl border border-zinc-200 bg-white p-1 lg:hidden" role="tablist" aria-label="Video workspace panels">
              {([['timeline', 'Timeline', FilmStrip], ['inspector', 'Inspector', SlidersHorizontal], ['takes', 'Takes', Stack]] as const).map(([value, label, Icon]) => <button type="button" key={value} role="tab" aria-selected={mobilePanel === value} onClick={() => setMobilePanel(value)} className={`flex h-9 flex-1 items-center justify-center gap-1.5 rounded-lg text-xs font-medium ${mobilePanel === value ? 'bg-violet-50 text-violet-700' : 'text-zinc-500'}`}><Icon size={14} />{label}</button>)}
            </div>
          </div>
        </main>
